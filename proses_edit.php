...
for ($j = 0; $j < $jumlah_soal; $j++) {
    ...
    $q[] = $qj;
    $pq[] = $pj * $qj;
    $r = korelasi($skor_soal, $total_tanpa_soal);
    $rHitung[] = $r;
...
for ($j = 0; $j < $jumlah_soal; $j++) {
    $atasBenar = array_sum(array_map(fn($s) => $s['row'][$j], $atas));
    $bawahBenar = array_sum(array_map(fn($s) => $s['row'][$j], $bawah));
